...
for (const { filename, pmName: pm } of countFiles) {
  ...
  if (countValues.length === 0) {
    throw new Error('No valid count values found in the temporary count file');
  }
...
// If no count file was found, throw an error
if (!pmName || !countData) {
  throw new Error('Could not read any temporary count file');
}
