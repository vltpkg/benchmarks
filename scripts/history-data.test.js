...
const {
  extractDayData,
  hasPartialResult,
} = require("../app/src/lib/history-data.ts");

const dataset = (data) => ({
  data: Object.fromEntries(
    Object.entries(data).map(([variation, rows]) => [
      variation,
      rows.map((row) => ({
...
test("registry median and completeness filters use the selected total-time source", () => {
  const response = {
    chartData: dataset({}),
    registryChartData: dataset({
      "registry-clean": [{ fixture: "next", npm: 3, vlt: 2 }],
    ...
    registryPerPackageCountChartData: dataset({
      "registry-clean": [
        { fixture: "next", npm: 90, vlt: 60, npm_partial: true },
      ],
    }),
  };
  assert.equal(hasPartialResult(response, ["registry-clean"], "npm"), false);
  response.registryChartData.data["registry-clean"][0].npm_partial = true;
  assert.equal(hasPartialResult(response, ["registry-clean"], "npm"), true);
  assert.deepEqual(extractDayData(response)["registry-clean"], { vlt: 2 });
  delete response.registryChartData.data["registry-clean"][0].vlt_statistic;
  assert.equal(extractDayData(response)["registry-clean"], undefined);
});
