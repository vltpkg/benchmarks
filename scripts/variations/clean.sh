...
hyperfine --ignore-failure \
  ...
  --warmup="$BENCH_WARMUP" \
  --runs="$BENCH_RUNS" \
  --prepare="bash $BENCH_SCRIPTS/clean-helpers.sh clean_all" \
  --conclude="bash $BENCH_SCRIPTS/package-count.sh $BENCH_OUTPUT_FOLDER; bash $BENCH_SCRIPTS/clean-helpers.sh clean_all" \
