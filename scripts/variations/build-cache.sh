...
BENCH_INSTALL_COMMAND_PNPM11="${BENCH_COMMAND_PNPM11//\$\{HYPERFINE_ITERATION\}/build-cache}"
...
BENCH_WARM_BUILD_PNPM11="timeout $BENCH_TIMEOUT corepack pnpm@next-11 run build > $BENCH_OUTPUT_FOLDER/pnpm11-warm-build-output.log 2>&1"
...
BENCH_COMMAND_PNPM11="timeout $BENCH_TIMEOUT corepack pnpm@next-11 run build > $BENCH_OUTPUT_FOLDER/pnpm11-build-output-\${HYPERFINE_ITERATION}.log 2>&1"
...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_YARN:+--prepare="$BENCH_INSTALL_PREPARE_YARN; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_YARN"} \
  ${BENCH_INCLUDE_YARN:+--command-name="yarn" "$BENCH_COMMAND_YARN"} \
  ${BENCH_INCLUDE_BERRY:+--prepare="$BENCH_INSTALL_PREPARE_BERRY; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_BERRY"} \
  ${BENCH_INCLUDE_BERRY:+--command-name="berry" "$BENCH_COMMAND_BERRY"} \
  ${BENCH_INCLUDE_ZPM:+--prepare="$BENCH_INSTALL_PREPARE_ZPM; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_ZPM"} \
  ${BENCH_INCLUDE_ZPM:+--command-name="zpm" "$BENCH_COMMAND_ZPM"} \
  ${BENCH_INCLUDE_PNPM:+--prepare="$BENCH_INSTALL_PREPARE_PNPM; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_PNPM"} \
  ${BENCH_INCLUDE_PNPM:+--command-name="pnpm" "$BENCH_COMMAND_PNPM"} \
  ${BENCH_INCLUDE_PNPM11:+--prepare="$BENCH_INSTALL_PREPARE_PNPM11; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_PNPM11"} \
  ${BENCH_INCLUDE_PNPM11:+--command-name="pnpm11" "$BENCH_COMMAND_PNPM11"} \
  ${BENCH_INCLUDE_VLT:+--prepare="$BENCH_INSTALL_PREPARE_VLT; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_VLT"} \
  ${BENCH_INCLUDE_VLT:+--command-name="vlt" "$BENCH_COMMAND_VLT"} \
  ${BENCH_INCLUDE_BUN:+--prepare="$BENCH_INSTALL_PREPARE_BUN; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_BUN"} \
  ${BENCH_INCLUDE_BUN:+--command-name="bun" "$BENCH_COMMAND_BUN"} \
  ${BENCH_INCLUDE_DENO:+--prepare="$BENCH_INSTALL_PREPARE_DENO; bash $BENCH_SCRIPTS/clean-helpers.sh clean_build_output; $BENCH_WARM_BUILD_DENO"} \
