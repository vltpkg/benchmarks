...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_REG_JFROG:+--prepare="$BENCH_PREPARE_BASE && $BENCH_SETUP_REGISTRY_JFROG"} \
  ${BENCH_INCLUDE_REG_JFROG:+--command-name="jfrog" "$BENCH_COMMAND_JFROG"} \
  ${BENCH_INCLUDE_REG_JFROG:+--conclude="$BENCH_CONCLUDE_JFROG"}
