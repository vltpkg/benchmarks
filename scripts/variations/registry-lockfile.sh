...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_REG_VLT_AUTH:+--prepare="$BENCH_PREPARE_BASE && $BENCH_SETUP_REGISTRY_VLT_AUTH"} \
  ${BENCH_INCLUDE_REG_VLT_AUTH:+--command-name="vlt-auth" "$BENCH_COMMAND_VLT_AUTH_REG"} \
  ${BENCH_INCLUDE_REG_VLT_AUTH:+--conclude="$BENCH_CONCLUDE_VLT_AUTH_REG"} \
