...
run_registry_benchmark() {
  ...
  # Create a wrapper script for cleaning registry cache between runs
  cat > "$BENCH_OUTPUT_FOLDER/clean-registry-$registry_type.sh" << EOF
#!/bin/bash
set -e
# Clean package manager artifacts
bash "$BENCH_SCRIPTS/clean-helpers.sh" clean_all

# Stop and restart registry with clean cache
...
sleep 2
EOF
  chmod +x "$BENCH_OUTPUT_FOLDER/clean-registry-$registry_type.sh"
  
  # Run the benchmark suite
  echo "Running benchmarks with $registry_type registry (proxied)..."
  hyperfine --ignore-failure \
    --time-unit=millisecond \
    --export-json="$BENCH_OUTPUT_FOLDER/benchmarks-$registry_type.json" \
    --warmup="$BENCH_WARMUP" \
    --runs="$BENCH_RUNS" \
    --prepare="bash $BENCH_OUTPUT_FOLDER/clean-registry-$registry_type.sh" \
    --conclude="sleep 1; bash $BENCH_SCRIPTS/package-count.sh $BENCH_OUTPUT_FOLDER" \
    --cleanup="bash $BENCH_SCRIPTS/clean-helpers.sh clean_all; bash $BENCH_SCRIPTS/registry-helpers.sh stop $registry_type" \
  ...
  rm -f "$BENCH_OUTPUT_FOLDER/clean-registry-$registry_type.sh"
}
