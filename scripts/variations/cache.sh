...
hyperfine --ignore-failure \
  ...
  --prepare="bash $BENCH_SCRIPTS/clean-helpers.sh clean_lockfiles clean_node_modules clean_package_manager_files" \
  --conclude="bash $BENCH_SCRIPTS/package-count.sh $BENCH_OUTPUT_FOLDER; bash $BENCH_SCRIPTS/clean-helpers.sh clean_lockfiles clean_node_modules clean_package_manager_files" \
