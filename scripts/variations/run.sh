...
BENCH_INSTALL_COMMAND_PACQUET="${BENCH_COMMAND_PACQUET//\$\{HYPERFINE_ITERATION\}/run}"
...
BENCH_COMMAND_PACQUET="timeout $BENCH_TIMEOUT /tmp/pnpm12/bin/pnpm run test > $BENCH_OUTPUT_FOLDER/pacquet-run-output-\${HYPERFINE_ITERATION}.log 2>&1"
...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_PACQUET:+--prepare="$BENCH_INSTALL_PREPARE_PACQUET"} \
