...
# Manually exclude berry for now
eval "BENCH_INCLUDE_BERRY="
...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_NPM:+--prepare="$BENCH_INSTALL_COMMAND_NPM"} \
  ...
  ${BENCH_INCLUDE_NX:+--prepare="$BENCH_INSTALL_COMMAND_NPM"} \
  ${BENCH_INCLUDE_NX:+--command-name="nx" "$BENCH_COMMAND_NX"} \
  ${BENCH_INCLUDE_TURBO:+--prepare="$BENCH_INSTALL_COMMAND_NPM"} \
  ${BENCH_INCLUDE_TURBO:+--command-name="turbo" "$BENCH_COMMAND_TURBO"} \
  ${BENCH_INCLUDE_NODE:+--prepare="$BENCH_INSTALL_COMMAND_NPM"} \
